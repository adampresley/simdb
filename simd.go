...
type Driver struct {
	...
	isOpened          bool
	entityDealingWith Entity // keeps the entity the driver is dealing with, field will maintain only the last entity inserted or updated or opened
	wantSingleRecord  bool   // Clues certain methods into the fact the user wants a single entity, not an array
	mutex             *sync.Mutex
}
...
func New(fs afero.Fs, dir string) (*Driver, error) {
	...
	err := createDirIfNotExist(fs, dir)
	return driver, err
}
...
func (d *Driver) GetNextNumericID() int {
	d.queries = nil
	field, _ := d.entityDealingWith.ID()
